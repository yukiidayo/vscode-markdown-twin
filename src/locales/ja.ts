...
export const ja: TranslationType = {
  ...
  azureRegion: 'Azure リージョン',
  azureRegionPrompt: 'Azure Translator のリージョンを入力してください',
  azureRegionPlaceHolder: 'global, japaneast, eastus, eastasia',
  azureRegionSaved: (region: string) => `Azure リージョンを保存しました: ${region}`,
  noSavedKeys: '保存済みのAPIキーはありません。',
  delete: '削除',
  deleteApiKeyConfirm: (provider: string) => `${provider} の保存済みAPIキーを削除しますか？`,
  apiKeyDeleted: (provider: string) => `${provider} のAPIキーを削除しました。`,
  ...
  apiKeyNotSetForProvider: (provider: string) => `プロバイダー「${provider}」のAPIキーが設定されていません。「Markdown Twin: APIキーを設定/変更」コマンドから設定してください。`,
